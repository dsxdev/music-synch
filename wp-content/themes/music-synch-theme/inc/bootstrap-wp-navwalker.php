	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			*/
			...
			/***** THS 3.8.2018
			 * Damit Item klickbar ist auch wenn Item Dropdown hat
			 */
			if ( $args->has_children && $depth === 0 ) {
				//$atts['href']        = '#';
			...
			$item_output .= ( $args->has_children && $depth < 2 ) ? '</a><span class="plus"></span>' : '</a>';
			//Navwalker hat vorher nur plus hinzugefügt wenn Ebene 0 ist. Nun wenn Ebene kleiner 2 ist.
			//$item_output .= ( $args->has_children && 0 === $depth ) ? '</a><span class="plus '.$depth.'"></span>' : '</a>';
